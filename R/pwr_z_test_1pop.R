#' Power and sample size for z test for one population
#'
#' \code{pwr_z_test_1pop} computes the power and the sample size for testing
#' mean in a normal variable with known variance.
#'
...
#' @keywords hypothesis testing, power, significance level, z test, one
#' population, one variable, sample size
#' @return \code{pwr_z_test_1pop} returns a list with the following components:
#' \describe{
...
#' }
#'
#' @usage pwr_z_test_1pop(m, m0, sigma, n = NULL, pwr = NULL,
...
#' alternative = "two.sided", sig_level = 0.05)
pwr_z_test_1pop <- function(m, m0, sigma, n = NULL, pwr = NULL,
                        alternative = "two.sided", sig_level = 0.05) {
  ...
  if (is.null(pwr)) {
    pwr <- switch(alternative,
                  "two.sided" = n %>%
                    map_dbl(~  1 -
                              pnorm(qnorm(1 - sig_level / 2) - es * sqrt(.x)) +
                              pnorm(qnorm(sig_level / 2) - es * sqrt(.x))),
                  "less" = n %>%
                    map_dbl(~ pnorm(qnorm(sig_level) - es * sqrt(.x))),
                  "greater" = n %>%
                    map_dbl(~ 1 - pnorm(qnorm(1 - sig_level) -
                                          es * sqrt(.x)))
    )
  } else if (is.null(n)) {
                ...
                "two.sided" = {
                  pwr %>%
                    map_int(function(pwr) {
                      faux <- function(n) n %>%
                        map_dbl(~ (1 - pnorm(qnorm(1 - sig_level / 2) -
                                               es * sqrt(.x)) +
